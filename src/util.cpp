...
const char * const BITCOIN_CONF_FILENAME = "taler.conf";
const char * const BITCOIN_PID_FILENAME = "taler.pid";
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Litecoin Core") == std::string::npos) {
        ...
        strYear.replace(strYear.find("2016"), sizeof("2016")-1, "2011");
        strCopyrightHolders += "\n" + strYear + "The Litecoin Core developers";
    }
    // Check for untranslated substitution to make sure Bitcoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        std::string strYear = strPrefix;
        strYear.replace(strYear.find("2016"), sizeof("2016")-1, "2009");
