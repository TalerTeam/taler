...
*Setup instructions for a Gitian build of Taler Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Taler
...
- [Building Taler Core](#building-taler-core)
...
Clone the git repositories for taler and Gitian.
...
in the taler repository under 'Fetch and create inputs' to install sources which require
...
To build Taler Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the taler repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/taler/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/taler-project/taler
...
For example:
```bash
URL=https://github.com/thrasher-/taler.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit taler=${COMMIT} --url taler=${URL} ../taler/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit taler=${COMMIT} --url taler=${URL} ../taler/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit taler=${COMMIT} --url taler=${URL} ../taler/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the taler git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../taler/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/taler
SIGPATH=/some/root/path/taler-detached-sigs

./bin/gbuild --url taler=${BTCPATH},signature=${SIGPATH} ../taler/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/taler-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/taler-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/taler-osx-build.assert
```
...
[taler-project/gitian.sigs.tlr](https://github.com/taler-project/gitian.sigs.tlr/) repository, or if that's not possible create a pull
